...
object DateTimeScalar {
    val DateTime: GraphQLScalarType = GraphQLScalarType.newScalar()
        .name("DateTime")
        ...
        .coercing(object : Coercing<OffsetDateTime, String> {
            override fun serialize(dataFetcherResult: Any, graphQLContext: GraphQLContext, locale: Locale): String =
                when (dataFetcherResult) {
                    ...
                    is LocalDateTime -> dataFetcherResult.atOffset(ZoneOffset.UTC)
                        .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME)
                    is String -> dataFetcherResult
                    else -> throw CoercingSerializeException("DateTime으로 직렬화할 수 없는 타입: ${dataFetcherResult::class}")
                }

            override fun parseValue(input: Any, graphQLContext: GraphQLContext, locale: Locale): OffsetDateTime =
                when (input) {
                    is String -> runCatching { OffsetDateTime.parse(input, DateTimeFormatter.ISO_OFFSET_DATE_TIME) }
            ...
            override fun parseLiteral(
                ...
                when (input) {
                    is StringValue -> runCatching {
                        OffsetDateTime.parse(input.value, DateTimeFormatter.ISO_OFFSET_DATE_TIME)
                    }.getOrElse { throw CoercingParseLiteralException("유효하지 않은 DateTime 리터럴: ${input.value}") }
