 ...
 * `File.fileUri` 필드는 별도 필드 리졸버로 처리해, `Score.file`처럼 다른 도메인이 반환하는
...
class FileWebAdapter(
    ...
    @SchemaMapping(typeName = "File", field = "fileUri")
    fun fileUri(file: File): String = generateFileAccessUrlUseCase.execute(file.fileUri)
}
